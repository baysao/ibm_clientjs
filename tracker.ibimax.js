$(function() {
	window.piwikMediaAnalyticsAsyncInit = function() {
		...
		function VideojsPlayer(node, mediaType) {
			...
			var c = MA.element.getAttribute(node, 'data-piwik-category');
			var f = MA.element.getAttribute(node, 'data-piwik-format');
			...
			if (c || typeof c !== null || typeof(c) !== 'undefined') {
				_paq.push(['trackEvent', 'MediaContentCategories', c, title]);
			}
			if (f || typeof f !== null || typeof(f) !== 'undefined') {
				_paq.push(['trackEvent', 'MediaFormat', f, title]);
			}
			...
			player.on('error', function() {
				...
				if ( typeof(this.networkState()) !== 'undefined' ) {
					_paq.push(['trackEvent', 'MediaError', err.name, title]);
